    ...
    "environment": [
      {
        ...
        "value": "udacity-jovi-azure-sql"
      },
      {
        "name": "AZURE_DOTNET_APP",
        "value": "udacity-jovi-azure-dotnet-app"
      }
    ],
    "portMappings": [
      {
...
resource "aws_dynamodb_table" "jovi-dynamodb-table" {
  name           = "GameScores"
...
resource "aws_s3_bucket" "jovi-s3-bucket" {
  bucket = "jovi-s3-bucket"
  tags = {
    name       = "jovi-s3-bucket"
    environment = "production"
  }
